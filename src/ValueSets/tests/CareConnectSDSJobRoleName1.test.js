...
const valueSet = require('../../../src/ValueSets/CareConnectSDSJobRoleName1.js')

describe('Care Connect Valueset CareConnectSDSJobRoleName1', function () {
  describe('validateCode function', function () {
    ...
    it('code is invalid', function () {
      let result = valueSet.validateCode('https://fhir.hl7.org.uk/STU3/CodeSystem/CareConnect-SDSJobRoleName-1', 'foo', 'bar')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      expect(resultBool).toBe(false)
    })
    it('code is valid, display is invalid', function () {
      let result = valueSet.validateCode('https://fhir.hl7.org.uk/STU3/CodeSystem/CareConnect-SDSJobRoleName-1', 'R0010', 'bar')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
    ...
    it('code and display are valid', function () {
      let result = valueSet.validateCode('https://fhir.hl7.org.uk/STU3/CodeSystem/CareConnect-SDSJobRoleName-1', 'R0010', 'Medical Director')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      let resultDisplay = result.parameter.find(prm => prm.name === 'display').valueString
