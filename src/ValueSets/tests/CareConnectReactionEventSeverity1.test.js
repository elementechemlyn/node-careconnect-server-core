...
const valueSet = require('../../../src/ValueSets/CareConnectReactionEventSeverity1.js')

describe('Care Connect Valueset CareConnectReactionEventSeverity1', function () {
  describe('validateCode function', function () {
    ...
    it('code is invalid', function () {
      let result = valueSet.validateCode('http://hl7.org/fhir/reaction-event-severity', 'foo', 'bar')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      expect(resultBool).toBe(false)
    })
    it('code is valid, display is invalid', function () {
      let result = valueSet.validateCode('http://hl7.org/fhir/reaction-event-severity', 'mild', 'bar')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
    ...
    it('code and display are valid', function () {
      let result = valueSet.validateCode('http://hl7.org/fhir/reaction-event-severity', 'mild', 'Mild')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      let resultDisplay = result.parameter.find(prm => prm.name === 'display').valueString
