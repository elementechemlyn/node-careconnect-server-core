...
const codeSystem = require('../../../src/CodeSystems/CareConnectNHSDataDictionaryPersonStatedGender1.js')

describe('Care Connect CodeSystem CareConnectNHSDataDictionaryPersonStatedGender1', function () {
  describe('validateCode function', function () {
    ...
    it('code and display are valid', function () {
      let result = codeSystem.validateCode('1', 'Male')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      let resultDisplay = result.parameter.find(prm => prm.name === 'display').valueString
