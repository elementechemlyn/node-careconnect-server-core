...
const codeSystem = require('../../../src/CodeSystems/CareConnectMedicationPrescribingAgency1.js')

describe('Care Connect CodeSystem CareConnectMedicationPrescribingAgency1', function () {
  describe('validateCode function', function () {
    ...
    it('code is valid, display is invalid', function () {
      let result = codeSystem.validateCode('prescribed-at-gp-practice', 'bar')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
    ...
    it('code and display are valid', function () {
      let result = codeSystem.validateCode('prescribed-at-gp-practice', 'Prescribed at GP practice')
      let resultBool = result.parameter.find(prm => prm.name === 'result').valueBoolean
      let resultDisplay = result.parameter.find(prm => prm.name === 'display').valueString
  ...
  describe('lookup function', function () {
    ...
    it('code is valid', function () {
      let result = codeSystem.lookup('prescribed-at-gp-practice')
      let resultDisplay = result.parameter.find(prm => prm.name === 'display').valueString
      expect(resultDisplay).toBe('Prescribed at GP practice')
    })
  })
